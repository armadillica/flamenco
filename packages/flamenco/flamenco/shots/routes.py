...
perproject_blueprint = Blueprint('flamenco.shots.perproject', __name__,
...
def for_project(project, flamenco_props, task_id=None, shot_id=None):
    api = pillar_api()
    ...
    thumb_placeholder = flask.url_for('static_flamenco', filename='assets/img/placeholder.jpg')
    for shot in shots:
    ...
    tasks_for_shots = current_flamenco.shot_manager.tasks_for_shots(
        shots,
        flamenco_props.task_types.flamenco_shot,
    )

    # Append the task type onto which 'other' tasks are mapped.
    task_types = flamenco_props.task_types.flamenco_shot + [None]
    ...
    return render_template('flamenco/shots/for_project.html',
                           shots=shots,
...
@flamenco_project_view(extension_props=True)
def view_shot(project, flamenco_props, shot_id):
    if not request.is_xhr:
        return for_project(project, flamenco_props, shot_id=shot_id)
    ...
    return render_template('flamenco/shots/view_shot_embed.html',
                           shot=shot,
                           project=project,
...
def save(project, shot_id):
    ...
    current_flamenco.shot_manager.edit_shot(shot_id, **shot_dict)
...
def create_shot(project):
    shot = current_flamenco.shot_manager.create_shot(project)
...
def activities(project, shot_id):
    ...
    acts = current_flamenco.activities_for_node(shot_id)
    return flask.render_template('flamenco/shots/view_activities_embed.html',
                                 activities=acts)
